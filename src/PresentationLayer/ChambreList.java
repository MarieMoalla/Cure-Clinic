...
public class ChambreList extends JFrame {
	...
	public void deleteButtonMousePressed(java.awt.event.MouseEvent evt) throws Exception
	{
		...
		String patientQuery = "select * from lit where Chambreid="+table.getModel().getValueAt(row, 0);
	    Statement statement = conn.createStatement();
	    
		ResultSet res = statement.executeQuery(patientQuery);
		if (res.next())  JOptionPane.showMessageDialog(null, "Chambre a des lits on ne peut pas la supprimer", "Action Denied", 0);
		else 
		{
			String deleteQuery = "delete from Chambre where id="+table.getModel().getValueAt(row, 0);
			...
			JOptionPane.showMessageDialog(null, "Chambre Successfully deleted!", "Done", 1,icon);
	...
	public void getList() throws Exception
	{
		...
		{	
			...
			String tabData[] = {Integer.toString(id)};
